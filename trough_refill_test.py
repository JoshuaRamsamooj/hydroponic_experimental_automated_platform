...
def update_status():
    try:
        ...
        off_troughs = []
        # get troughs whose state is 'off'
...
def raise_sensor_alarm():
    ...
    # alarm1.send_alarm_email(alarm_description, alarm_priority, email)
...
def mix(runtime): 
    ...
    while time.time() < finish:
        ...
        if mixingpump_flow is False:
            # raise alarm 
            # alarm_description = "Mixing pump failure."
...
def rinse():
    ...
    # if rinse_water is False: # means there is issue with pump

    #     print "mixing tank failed"
    #     # raise an alarm
    #     # alarm_description = "Failure of Water pump."
    #     alarm_description = "Failure of Pump P11 (Water Pump). Manually rinse mixing tank before clearing alarm."
    #     alarm_priority = "High"
    #     alarm_type = "4,5"
    #     affected_equipment = troughs_affected("water")+",pump_water"
    #     email = str(heap1.get_heapdata('alarm_settings', 'email'))
    ...
    if height is False:
        ...
        # while heap1.get_level("tank_mixing")[0] >= 2:
            ...
            if drain_flow is False:            
                # alarm_description = "Failure of drain valve. Manually remove contents from mixing tank."
...
def dose_in_sections(number_of_times_to_dose, water_height, n_req, p_req, k_req, mn_req, sman_req, trough):
    ...
    # every time alarm is raised...return false
    for _ in xrange(number_of_times_to_dose):
        # at this point, trough will be empty 
        ...
        if n_req != 0:  # accouts for the def in the particular chemical
            ...
            if n_added is False: # means there is issue with pump
                ...
                # raise an alarm
                # alarm_description = "Failure of Nitrogen dosing pump."
        ...
        if p_req != 0:  # accouts for the def in the particular chemical
            ...
            if p_added is False: # means there is issue with pump
                ...
                # raise an alarm
                # alarm_description = "Failure of phosphorus dosing pump."
        ...
        if k_req != 0:  # accouts for the def in the particular chemical
            ...
            if k_added is False: # means there is issue with pump
                ...
                # raise an alarm
                # alarm_description = "Failure of potassium dosing pump."
        ...
        if mn_req != 0:  # accouts for the def in the particular chemical
            ...
            if mn_added is False: # means there is issue with pump
                ...
                # raise an alarm
                # alarm_description = "Failure of micro-nutrients dosing pump."
        ...
        if sman_req != 0:  # accouts for the def in the particular chemical
            ...
            if sman_added is False: # means there is issue with pump
                ...
                # raise an alarm
                # alarm_description = "Failure of seconday macro-nutrients dosing pump."
        ...
        if mix_solution is False:
            drain_tank()
            # if drain_tank() is False: return False
            # cant rinse if mixing pump gone
            return False 
...
def shutdown():
    ...
    # set air pump off in db
    # set plant lights off in db
...
while True:
    ...
    for trough in range(1,6):
        ...
        # put level checks here for tanks
        # just to give real time on hmi
        ...
        #use a pause variable
        while int(heap1.get_heapdata("system_variables", "system_pause")) == 1:
            pass

        # set dosing in progress indicator OFF here
        # used when shutting down or manual dosing
        heap1.set_heapdata('system_variables', 'dosing_in_progess', '0')
        ...
        #check for overflow 
        #unnecessary - just have manual overflow pipe in build
        # if trough_level > max_level:
        #     alarm_description = "Trough "+ str(trough) +" overflow."
        #     alarm_priority = "Low"
        #     alarm_type = "1"
        #     affected_equipment = "trough_"+str(trough)
        #     email = heap1.get_heapdata('alarm_settings', 'email')
        #     alarm1.log_alarm(alarm_description, alarm_priority, alarm_type, affected_equipment, email)
            alarm1.send_alarm_email(alarm_description, alarm_priority, email)
        #     continue
        ...
        if trough_level < threshold_level:
            ...
            # ml_to_refill = ml_to_refill + 1000  # add the error causes by the outlets so the ratio will stay the same
            ...
            # do water first...that should be the most 
            ...
            # check to see if you have enough of each to refill the mixture
            ...
            # once code reaches here it means you have enough nutrients to mix
            # get total volume to mix
            # that's ml_to_refill
            # check if its more than the tanks capacity 
            ...
            # n_req = (float(water_req) / float(water)) * n  # ml values
            # p_req = (float(water_req) / float(water)) * p  # ml values
            # k_req = (float(water_req) / float(water)) * k  # ml values
            # mn_req = (float(water_req) / float(water)) * mn  # ml values
            # sman_req = (float(water_req) / float(water)) * sman  # ml values
    ...
    # make this changable in the hmi 
